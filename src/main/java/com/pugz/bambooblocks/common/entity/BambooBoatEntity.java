...
import com.pugz.bambooblocks.core.registry.EntityRegistry;
import com.pugz.bambooblocks.core.registry.ItemRegistry;
...
public class BambooBoatEntity extends BoatEntity {
    ...
    public BambooBoatEntity(EntityType<? extends BoatEntity> type, World world) {
        super(type, world);
        preventEntitySpawning = true;
    }

    public BambooBoatEntity(World worldIn, double x, double y, double z) {
        this(EntityRegistry.BAMBOO_BOAT, worldIn);
        setPosition(x, y, z);
    ...
    public BambooBoatEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
        this(EntityRegistry.BAMBOO_BOAT, world);
    }
    ...
    protected void registerData() {
        ...
        dataManager.register(BOAT_TYPE, Type.BAMBOO.ordinal());
    ...
    public Item getItemBoat() {
        switch (getBoatModel()) {
            default:
            case BAMBOO:
                return ItemRegistry.BAMBOO_BOAT;
        }
    }
        ...
        if (!isPassenger()) {
            if (onGroundIn) {
                if (fallDistance > 3.0F) {
                    if (status != BambooBoatEntity.Status.ON_LAND) {
                        fallDistance = 0.0F;
                        return;
                    }
    ...
    public enum Type {
        BAMBOO(BlockRegistry.BAMBOO_PLANKS, "bamboo");
